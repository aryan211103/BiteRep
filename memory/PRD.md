...
- Multi-step onboarding: name → sex → age → height/weight (imperial default, metric toggle) → activity → goal → faith & diet flags → summary. Live BMR → maintenance → adjustment → target math preview using Mifflin-St Jeor.
