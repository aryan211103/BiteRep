...
- Food search quality: relevance ranking (query tokens must appear in name/brand), de-dup keeping most-scanned product, real serving size/label/kcal (not just per-100g), US-biased search with global fallback if <8 relevant results.
- AI Photo Meal Logging (hero): snap/upload a meal photo → Gemini vision identifies each dish + estimates grams/macros → backend grounds each item against Open Food Facts (green "From database" badge) or keeps the AI estimate (amber badge) → editable grams stepper with live recompute, running total, meal picker → photo persisted via Emergent Object Storage and attached to each food_log (`photo_path`).
- Label scanning (/label-scan): photograph a Nutrition Facts panel → Gemini vision extracts name/brand/serving/per-100g macros → editable confirm form → saved to a personal `saved_foods` collection (shown first, tagged "Saved", in future `/foods/search` results) and optionally logged immediately.
- Barcode scanning (/barcode-scan): expo-camera `CameraView` live barcode scan → `GET /api/foods/barcode/{code}` (Open Food Facts product lookup) → one-tap add to a meal.
- Recipes: "Save as recipe" on any Home meal section bundles its current logged items into a reusable `recipes` doc; a "My Recipes" screen (from Profile) lists them with one-tap "Log again" (re-creates food_log entries for a chosen meal/date) and delete.
- Weekly Recap: Trends tab "This week" card (`GET /api/trends/weekly-recap`) shows 7-day avg calories vs target, adherence %, mini bar chart, and best-protein-day callout.
